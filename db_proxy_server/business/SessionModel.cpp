...
bool Judgment(uint32_t nUserId,uint32_t nPeerId, IM::BaseDefine::SessionType nSessionType );
    ...
    if (pDBConn)
    {
        ...
        if (pResultSet)
        {
            while (pResultSet->Next())
            {
				...
				// add  7.3 
				log("befor judgment ");
				if(!Judgment( nUserId, nPeerId, nSessionType ))
						continue;
        ...
        if(!lsContact.empty())
        {
			log(" lsContact.size = %d ", lsContact.size());
...
bool Judgment(uint32_t nUserId,uint32_t nPeerId, IM::BaseDefine::SessionType nSessionType )
{
    ...
    {
	string strSqlTmp;
		if( IM::BaseDefine::SESSION_TYPE_SINGLE == nSessionType )
			{
				 strSqlTmp = "select status from IMUser where id=" + int2string(nPeerId);//查询该用户是否被删除
				log("strSqlTmp =  %s ",strSqlTmp.c_str());
			}
		else if(IM::BaseDefine::SESSION_TYPE_GROUP == nSessionType)
			{
			 	strSqlTmp = " select status from IMGroupMember where groupId="+int2string(nPeerId) + " and userId="+int2string(nUserId) ;//查询该用户是否还在群里
				log(" group strSqlTmp =  %s ",strSqlTmp.c_str());
			}
