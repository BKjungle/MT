...
public:
    ...
    BaseTransferTask* NewTransferTask(uint32_t trans_mode, const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id, const std::string& file_name, uint32_t file_size,const std::string& file_md5);
	...
	// add end 
	bool IsTaskInTasks(BaseTransferTask* transfer_task){
			
		 for (TransferTaskMap::iterator it = transfer_tasks_.begin(); it != transfer_tasks_.end(); ) {
        		TransferTaskMap::iterator it_old = it;
        		it++;
        
        		BaseTransferTask* transfertask = (BaseTransferTask*)it_old->second;

					if(transfertask == transfer_task){
						return true;
					}
   		}
		 	return false;
