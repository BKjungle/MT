...
void FileMsgServerConn::_HandleMsgFileTransferReq(CImPdu* pdu) {
    ...
    do {
        ...
        log("trams_mode=%d, task_id=%s, from_id=%d, to_id=%d, file_name=%s, file_size=%ld", transfer_req.trans_mode(), task_id.c_str(), from_id, to_id, transfer_req.file_name().c_str(), transfer_req.file_size());
...
void FileMsgServerConn::_HandleGetServerAddressReq(CImPdu* pPdu) {
    ...
    const std::list<IM::BaseDefine::IpAddr>& addrs_peer = ConfigUtil::GetInstance()->GetPeerAddressList(); // add  6.15
    for (std::list<IM::BaseDefine::IpAddr>::const_iterator it=addrs_peer.begin(); it!=addrs_peer.end(); ++it) {
...
//add send pdu  to  msgserver  6.13 
void Send_to_msgserver(CImPdu* pPdu)
{
	int ret = -1;
	ConnMap_t::iterator it = g_file_msg_server_conn_map.begin();
	log(" g_file_msg_server_conn size  = %d ", g_file_msg_server_conn_map.size());
	if(it->second)
		{
			CImConn* conn = it->second;
			

			ret = conn->SendPdu(pPdu) ;
			if(ret > 0 );
			log(" SendPdu ok  ret = %d ",ret );
		}
}
