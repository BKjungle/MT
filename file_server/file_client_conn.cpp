...
#include "base/pb/protocol/IM.BaseDefine.pb.h" //add 6.16. for send download ok to msgserver
...
void FileClientConn::Close() {
    ...
    if (transfer_task_) {
	       ...
	       if (transfer_task_->state() >= kTransferTaskStateUploadEnd) {
					log(" state = %d ",transfer_task_->state());
...
void FileClientConn::_HandleClientFileLoginReq(CImPdu* pdu) {
    ...
    log("Client login, user_id=%d, task_id=%s, file_role=%d, seqnum =  %d ", user_id, task_id.c_str(), mode,pdu->GetSeqNum());
    ...
    if (rv) {
        if (transfer_task->GetTransMode() == FILE_TYPE_ONLINE) {
            if (transfer_task->state() == kTransferTaskStateWaitingTransfer) {
                ...
                if (conn) {
                    _StatesNotify(CLIENT_FILE_PEER_READY,FILE_UPLOAD, task_id, transfer_task_->from_user_id(), conn);
                } else {
...
void FileClientConn::_HandleClientFileStates(CImPdu* pdu) {
    ...
    uint32_t file_stat = file_state.state();
   	uint32_t Transfer_type = file_state.ttype();  
    log("Recv FileState, user_id=%d, task_id=%s, file_stat=%d,Transfer_type = %d ", user_id, task_id.c_str(),file_stat,Transfer_type);
...
void FileClientConn::_HandleClientFilePullFileReq(CImPdu *pdu) {
    ...
    if (rv!=0) {
  //      Close(); //   ----------------  this  change  is very important , don‘t  forget it  //  6.26
  		
        log(" rv = %d  and not close ",rv ); // add 6.26
    }
...
void FileClientConn::_HandleClientFilePullFileRsp(CImPdu *pdu) {
    ...
    do {
        ...
        if (transfer_task_->GetTransMode() == FILE_TYPE_ONLINE) {
            ...
            // all packages recved
            if (rv == 1) {
                _StatesNotify(CLIENT_FILE_DONE,FILE_UPLOAD, task_id, user_id, this);
                // Close();
            } else {
...
int FileClientConn::_StatesNotify(int state, int TType,const std::string& task_id, uint32_t user_id, CImConn* conn) {
	...
	file_msg.set_ttype(static_cast<TransferType>(TType));   	
