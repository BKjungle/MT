...
int main(int argc, char* argv[])
{
	...
	char* str_client_peer_listen_ip = config_file.GetConfigName("clientPeerListenIp");//add 6.15
	char* str_client_peer_listen_port = config_file.GetConfigName("clientPeerListenPort");//add 6.15	
	...
	uint16_t client_peer_listen_port = atoi(str_client_peer_listen_port);//add 6.15

    CStrExplode client_listen_ip_list(str_client_listen_ip, ';');
	CStrExplode client_peer_listen_ip_list(str_client_peer_listen_ip,';');//add 6.15
	...
	for (uint32_t j = 0; j < client_peer_listen_ip_list.GetItemCnt(); j++) {
        ConfigUtil::GetInstance()->Add_Peer_Address(client_peer_listen_ip_list.GetItem(j), client_peer_listen_port);
    }
