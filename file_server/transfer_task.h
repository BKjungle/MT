...
struct OfflineFileHeader {
	...
	void set_file_md5(const char* p) {
		sprintf(file_md5, p, 512 < strlen(p) ? 512 : strlen(p));
	  }
...
public:
    BaseTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id, const std::string& file_name, uint32_t file_size,const std::string& file_md5);
	...
	inline const std::string& file_md5() const { return file_md5_; }		//-------add md5
...
public:
    OnlineTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id, const std::string& file_name, uint32_t file_size,const std::string& file_md5)
...
public:
    OfflineTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id, const std::string& file_name, uint32_t file_size,const std::string& file_md5)
        : BaseTransferTask(task_id, from_user_id, to_user_id, file_name, file_size,file_md5) {
//        file_header_ = NULL;
        fp_ = NULL;
		fp_mobile = NULL;
