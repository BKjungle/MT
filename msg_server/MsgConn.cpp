...
void CMsgConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId()) {
        ...
        case CID_FILE_DEL_OFFLINE_REQ:
			...
			break;
		case CID_FILE_Notify__Ack: 								// add 6.21 
			s_file_handler->HandleNotifyAck(pPdu);
			break;
        default:
...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
    ...
    pdu.SetSeqNum(pPdu->GetSeqNum());
    int numb = pDbConn->SendPdu(&pdu);
	log(" numb = %d, pdulen = %d ",numb ,pPdu->GetLength());
}
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu)
{
	...
	log(" msg.data  length = %d ",msg.msg_data().length());// add 5.8
...
void CMsgConn::_HandleClientMsgDataAck(CImPdu* pPdu)
{
	...
	log(" client MsgDataAck session_id = %d,usrid = %d",msg.session_id(), GetUserId());
}
