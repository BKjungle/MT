...
{
	log(" command =  %d",pPdu->GetCommandId());
...
void CFileServConn::_HandleFileMsgTransRsp(CImPdu* pPdu)
{
    ...
    if (result == 0)
    {
        ...
        if( FILE_TYPE_ONLINE == trans_mode )
			{
...
IM::BaseDefine::InfoNotify NotifyMsg;
	...
	IM::BaseDefine::OfflineFileInfo Qinfo =  NotifyMsg.offline_file_list(0);
	uint32_t fromid = Qinfo.from_user_id();
	string file_name  = Qinfo.file_name();
	uint32_t status = Qinfo.status();
	log(" fromid = %d, file_name = %s , status = %d ",fromid,file_name.c_str(),status );

	IM::File::IMFileHasOfflineRsp   downfile ;
	
	downfile.set_user_id(fromid);
	IM::BaseDefine::OfflineFileInfo* temp =  downfile.add_offline_file_list();
	temp->set_from_user_id(fromid);
	temp->set_task_id(Qinfo.task_id());
	temp->set_file_name(file_name);
	temp->set_file_size(Qinfo.file_size());
	temp->set_file_md5(Qinfo.file_md5());
	...
	const list<IM::BaseDefine::IpAddr> * iptemp  = NULL;
		...
		CImUser* fromusr  = CImUserManager::GetInstance()->GetImUserById(fromid);
	    if (fromusr) {
	...
	for(int i = 0;i< infonotify.offline_file_list_size(); i++)
		{
			...
			log(" CID_FILE_Notify_dbserver file_name =  %s, size = %d , status = %d  ", fileinfo.file_name().c_str(),fileinfo.file_size(),fileinfo.status());
		}
